...
public class AppliedBotanicsForge {
    ...
    public AppliedBotanicsForge(IEventBus bus) {
        ...
        bus.addListener((RegisterPartCapabilitiesEvent event) -> {
            event.register(BotaniaNeoForgeCapabilities.getBlockApiLookupById(ManaReceiver.LOOKUP),
                    (object, context) -> object.getExposedApi(),
                    ManaP2PTunnelPart.class);
            event.register(BotaniaNeoForgeCapabilities.getBlockApiLookupById(ManaSparkAttachable.LOOKUP),
                    (object, context) -> object.getManaSparkAttachable(),
                    ManaP2PTunnelPart.class);
        });
    ...
    private void registerGenericAdapters(RegisterCapabilitiesEvent event) {
        for (var block : BuiltInRegistries.BLOCK) {
            ...
            event.registerBlock(BotaniaNeoForgeCapabilities.getBlockApiLookupById(ManaReceiver.LOOKUP),
            ...
            event.registerBlock(BotaniaNeoForgeCapabilities.getBlockApiLookupById(ManaSparkAttachable.LOOKUP),
                    (level, pos, state, blockEntity, context) -> {
                        // assume the spark is accessing the up face (might not work with spark
                        // tinkerer)
                        var genericInv = level.getCapability(AECapabilities.GENERIC_INTERNAL_INV, pos, state,
        ...
        for (var item : BuiltInRegistries.ITEM) {
            ...
            event.registerItem(BotaniaNeoForgeCapabilities.getItemApiLookupById(ManaItem.LOOKUP), (object, context) -> {
